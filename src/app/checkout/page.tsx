...
export default function CheckoutPage() {
  ...
  const [whatsappUrl, setWhatsappUrl] = useState<string | null>(null); // State for the final URL
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Set the final URL in the state instead of opening it directly
      setWhatsappUrl(`https://wa.me/${phone}?text=${message}`);

      setToast("Order saved! Click the button below to confirm on WhatsApp.");
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                ...
                {!whatsappUrl ? (
                  <>
                    ...
                    >
                      {isProcessing ? 'Saving Order...' : 'Confirm Order'}
                    </button>
                  </>
                ) : (
                  <a
                    ...
                    onClick={() => {
                      // Clean up the UI after the user clicks the link
                      localStorage.removeItem("cart");
                      setCart([]);
                      setTimeout(() => router.push("/"), 500);
                    }}
                    className="w-full text-center px-6 py-3 rounded-xl font-semibold bg-green-600 text-white hover:bg-green-700"
                  >
